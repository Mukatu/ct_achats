...
async function submitForm() {
  ...
  try {
    ...
    const response = await contratService.create(data)
    router.push(`/contrats/${response.data.data.id}`)
  } catch (error: any) {
  ...
  <div>
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        ...
        <p class="text-gray-600 mt-1">Creer un nouveau contrat recurrent</p>
      </div>
    </div>
    ...
    <form v-else @submit.prevent="submitForm" class="space-y-6">
      ...
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p v-if="errors.type_contrat_id" class="text-red-500 text-sm mt-1">{{ errors.type_contrat_id[0] }}</p>
          </div>
          <div>
          ...
          <div>
            <label class="label">Reference externe (n contrat fournisseur)</label>
            <input v-model="form.reference_externe" type="text" class="input" placeholder="Ex: CTR-FOURNISSEUR-001" />
          </div>
          <div>
            <label class="label">Contact fournisseur</label>
            <input v-model="form.contact_fournisseur" type="text" class="input" placeholder="Nom et coordonnees" />
          </div>
          <div class="md:col-span-2">
            <label class="label">Objet du contrat *</label>
            <input v-model="form.objet" type="text" class="input" :class="{ 'border-red-500': errors.objet }" placeholder="Description courte du contrat" />
            <p v-if="errors.objet" class="text-red-500 text-sm mt-1">{{ errors.objet[0] }}</p>
          </div>
          <div class="md:col-span-2">
            <label class="label">Description detaillee</label>
            <textarea v-model="form.description" class="input" rows="3" placeholder="Details supplementaires..."></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input v-model="form.date_signature" type="date" class="input" :class="{ 'border-red-500': errors.date_signature }" />
            <p v-if="errors.date_signature" class="text-red-500 text-sm mt-1">{{ errors.date_signature[0] }}</p>
          </div>
          <div>
            ...
            <p v-if="errors.date_debut" class="text-red-500 text-sm mt-1">{{ errors.date_debut[0] }}</p>
          </div>
          <div>
            <label class="label">Date de fin (laisser vide si CDI)</label>
          ...
          <div v-if="form.date_fin">
            ...
            <input v-model.number="form.preavis_jours" type="number" min="0" class="input" placeholder="Ex: 90" />
          </div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input v-model.number="form.montant_periodique" type="number" min="0" class="input" :class="{ 'border-red-500': errors.montant_periodique }" placeholder="Montant par periode" />
            <p v-if="errors.montant_periodique" class="text-red-500 text-sm mt-1">{{ errors.montant_periodique[0] }}</p>
          </div>
          <div>
          ...
          <div>
            <label class="flex items-center gap-2 cursor-pointer mt-6">
              ...
              <span class="text-sm text-gray-700">TVA incluse dans le montant</span>
            </label>
          </div>
          <div>
            <label class="label">Jour de facturation</label>
            <input v-model.number="form.jour_facturation" type="number" min="1" max="31" class="input" placeholder="1-31" />
          </div>
          <div class="md:col-span-3">
            <label class="label">Conditions de paiement</label>
            <input v-model="form.conditions_paiement" type="text" class="input" placeholder="Ex: A 30 jours fin de mois" />
          </div>
        </div>
      </div>
        ...
        <div class="space-y-4">
          <div>
            <label class="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" v-model="form.generer_echeances" class="w-4 h-4 text-ct-blue-600 rounded" />
              <span class="text-sm text-gray-700">Generer automatiquement les echeances</span>
            </label>
            <p class="text-xs text-gray-500 ml-6 mt-1">Les echeances seront generees selon la periodicite jusqu'a la date de fin</p>
          </div>
          <div>
            <label class="label">Commentaire</label>
            <textarea v-model="form.commentaire" class="input" rows="2" placeholder="Notes internes..."></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-4">
        ...
        <button type="submit" :disabled="saving" class="btn-primary inline-flex items-center">
          <DocumentPlusIcon v-if="!saving" class="w-5 h-5 mr-2" />
