...
async function loadReferentiels() {
  loading.value = true
  try {
    const response = await referentielService.getServices()
...
async function submitForm() {
  ...
  try {
    ...
    if (!data.seuil_validation) {
      delete (data as any).seuil_validation
    }
  ...
  <div>
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        ...
        <p class="text-gray-600 mt-1">Creer un nouveau compte utilisateur</p>
      </div>
    </div>
    ...
    <form v-else @submit.prevent="submitForm" class="space-y-6">
      ...
      <div class="flex justify-end gap-4">
        ...
        <button type="submit" :disabled="saving" class="btn-primary inline-flex items-center">
          ...
          {{ saving ? 'Enregistrement...' : 'Creer l\'utilisateur' }}
        </button>
      </div>
    </form>
  </div>
